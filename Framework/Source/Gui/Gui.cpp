...
namespace Trinity
{
	...
	bool Gui::create(Window& window, const std::string& defaultFontPath)
	{
		ImGui::CreateContext();
	...
	void Gui::newFrame(Window& window, float deltaTime)
	{
		ImGuiIO& io = ImGui::GetIO();
	...
	void Gui::draw(ImDrawData* drawData, RenderPass& renderPass)
	{
	...
	void Gui::setupCallbacks(Window& window)
	{
	...
	void Gui::updateMouseCursor(Window& window)
	{
		ImGuiIO& io = ImGui::GetIO();
	...
	void Gui::setupRenderStates(ImDrawData* drawData, RenderPass& renderPass)
	{
	...
	bool Gui::createCommonBindGroup()
	{
	...
	bool Gui::createImageBindGroup(const Texture& texture)
	{
